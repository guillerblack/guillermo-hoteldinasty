...
const DetallesHabitacion = () => {
  ...
  const { nombre, descripcion, valor } = habitacion;
  // Provide default empty array for facilities if it doesn't exist
  const facilities = habitacion.facilities || [];
  
  let imageSrc = habitacion.image || '/img/default-room.jpg';
  
  // Si la imagen es null, usar la imagen por defecto
  if (!imageSrc || imageSrc === 'null') {
    imageSrc = '/img/default-room.jpg';
  }
  
  // Agregar la URL base del servidor
  if (imageSrc && !imageSrc.startsWith('http')) {
    imageSrc = `http://localhost:5000${imageSrc}`;
  }
  
  console.log('Ruta de imagen completa:', imageSrc);
            ...
            <div className="w-full h-full lg:w-[60%] px-6 ">
              ...
              <img 
                ...
                onError={(e) => {
                  ...
                  e.target.src = 'http://localhost:5000/img/default-room.jpg';
                }}
              />
              {/* Instalaciones */}
              <div className="mt-12">
                ...
                <div className="grid grid-cols-3 gap-6 mb-12">
                  {facilities.length > 0 ? (
                    ...
                    <div className="col-span-3">
                      <p>No hay instalaciones disponibles para mostrar.</p>
                    </div>
                  )}
                </div>
              </div>
